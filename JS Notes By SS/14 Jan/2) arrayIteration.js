...
const numbs = [1, 2, 3, 4, 5, 12, 32, 22, 23, 43, 34, 555, 64, 51, 123, 1234, 4321, 321, 231, 244, 432, 51, 632];
console.log("Universal Numbers Array: " + numbs);


const cities = ["Pune", "Nashik", "Pune", "Mumbai", "Nagpur", "Nandurbar", "Dhule", "Pune", "Pune"];
...
// We dont need to provide the arguments in the callback function here because the forEach will automatically add the arguments. But is is necessary to provide the parameters in the function definition to make it work.
...
// The call back function in this method works similar to the filter method. Thus returns the boolean value. The only difference here is that the arr.filter() gives the final array and arr.every() returns the final boolean value. 
// This example check if all array values are larger than 10
// Note that here the map/filter differ with every because the every() method after completion of the itteration does not returns the complete array rather it returns true or falsr as the result.
...
// Everything is same in respect of some() and every() methotds. Except that the sone() method returns true even if some of the elements satisfy the given conditions.
