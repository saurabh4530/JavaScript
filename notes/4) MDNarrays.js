...
console.log(Array.at.call(arrayLike, -1)); // "b"
...
// This above push('z') will change the value in array because the first nested array stored in the array1 is not
// a value rather it is the reference to that nested array in memory.
// Therefore when concatination is done the new array created using concat will have values as values only 
//but this nested array is stored as a reference. Therefore any updates will be reflected in the concatinated array also.
...
const iterator1 = arrayItr.entries();
...
// Therefore iterator can only traverse once.
...
// Code block below will not work because the array itterator only work once thus the program will not at all enter in the for loop
// because the arrayEntries becomes empty if any element is itterated once.
